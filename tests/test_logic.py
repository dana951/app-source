...
def test_safe_css_invalid_uses_theme_env(mocker: MockerFixture) -> None:
    mocker.patch(
...
def test_safe_css_invalid_defaults_to_blue(mocker: MockerFixture) -> None:
    mocker.patch(
        "podinfo.logic.os.getenv",
...
def test_safe_css_invalid_theme_falls_back_to_blue(mocker: MockerFixture) -> None:
    mocker.patch(
        "podinfo.logic.os.getenv",
...
def test_get_version(mocker: MockerFixture, pod: PodInfo) -> None:
    mocker.patch(
        "podinfo.logic.os.getenv",
    ...
    assert pod.get_version() == {"version": "9.9.9", "git_sha": "a1b2c3d4"}
...
def test_get_info(mocker: MockerFixture, pod: PodInfo) -> None:
    ...
    mocker.patch("podinfo.logic.socket.gethostname", return_value="test-host")
    mocker.patch("podinfo.logic.platform.platform", return_value="TestOS-1.0")
    assert pod.get_info() == {
        "hostname": "test-host",
        "platform": "TestOS-1.0",
        "environment": "staging",
...
def test_get_dashboard_html_keys(mocker: MockerFixture, pod: PodInfo) -> None:
    mocker.patch(
        "podinfo.logic.os.getenv",
        side_effect=mock_getenv(env_is_empty=True),
    )
    mocker.patch("podinfo.logic.socket.gethostname", return_value="h1")
...
def test_echo_message(
    ...
    mocker.patch("podinfo.logic.datetime.now", return_value=mock_now)
